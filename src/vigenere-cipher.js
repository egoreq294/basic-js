    ...
    encrypt(message, key) {
        ...
        for (let i = 0; i < message.length; i++) {
            if (message[i].match(/[A-Za-z]/)!=null) {
                ...
                data.result[i] = String.fromCharCode(( codeCharSymbol + (key[data.index % key.length].toUpperCase().charCodeAt()) - 65 * 2) % 26 + 65);
                data.index++;
            } else {
    ...
    decrypt(encryptedMessage, key) {
        ...
        for (let i = 0; i < encryptedMessage.length; i++) {
            if (encryptedMessage[i].match(/[A-Za-z]/)!=null) {
                ...
                data.result[i] = String.fromCharCode(Math.abs(codeCharSymbol - (key[data.index % key.length].toUpperCase().charCodeAt()) + 26) % 26 + 65);
                data.index++;
            } else {
